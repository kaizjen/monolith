...
<script>
  ...
  const available = window.monolith.i18n.getAvailableTranslations();
  ...
  let availableDialog = location.hash.startsWith('#language-for-monolith')

  let needsRestart = false;

  function selectMonolithLanguageF(code) {
    return function () {
  ...
  function selectChromiumLanguageF(code) {
    return function () {
      ...
      if (window.monolith.i18n.getSupportedLanguage(code) == code) {
        $config.i18n.lang = code;
      }
      update()
...
<ContentDialog bind:open={availableDialog} style="max-height: 100%; overflow: auto;" title={tt('dialog')}>
  {#each available as code}
    <RadioButton group={$config.i18n.lang} value={code} on:input={selectMonolithLanguageF(code)}>
...
<div class="s-option">
  <TextBlock>{tt('label-monolith')}</TextBlock>
...
{#if needsRestart}
  <div class="s-option">
    <InfoBar severity="caution" title={tt('restart-notification')}>
      <Button slot="action" variant="accent" on:click={window.monolith.app.restart}>
        {tt('restart-button')}
      </Button>
    </InfoBar>
  </div>
