...
<script>
  ...
  $: promise = new Promise(async y => {
    bookmarks = await window.monolith.userdata.bookmarks.getFolder({ folder: selectedFolder });
    y();
  })
  ...
  function delF(i) {
    return async () => {
      ...
      await window.monolith.userdata.bookmarks.setFolder({ folder: selectedFolder, value: bookmarks });
      bookmarks = bookmarks;
    }
  }

  async function deleteFolder() {
    await window.monolith.userdata.bookmarks.delFolder({ folder: selectedFolder });
    update()
  }
  ...
  async function rename() {
    ...
    await window.monolith.userdata.bookmarks.renFolder({ folder: selectedFolder, name: newFolderName });
    await update()
