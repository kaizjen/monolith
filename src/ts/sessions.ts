...
export function registerSession(ses: Session) {
  ...
  ses.protocol.registerFileProtocol('mth', (req, respond) => {
...
export const CHROME_PARTITION = 'chrome'

app.once('ready', () => {
  session.fromPartition(NO_CACHE_PARTITION, { cache: false }) // create a non-cacheable session
  session.fromPartition(CHROME_PARTITION, { cache: false })
  ...
  session.fromPartition(CHROME_PARTITION).protocol.registerStringProtocol('m-res', async(req, respond) => {
    if (req.url.includes('..')) {
      respond({ error: -10 }) // Access denied
      return;
    }
