...
export function init() {
  ...
  function actArgv(argv: ReturnType<ArgvParse>, workingDir: string, shouldOpenWindow?: boolean) {
    if (argv._?.[0]) {
      ...
      let url: string;
      if (urlOrPath.startsWith('http:') || urlOrPath.startsWith('https:') || urlOrPath.startsWith('mth:') || urlOrPath.startsWith('ftp:')) {
        url = argv._[0]
  ...
  if (argv['second-instance'] || app.requestSingleInstanceLock({ argv: JSON.stringify(argv) })) {
    ...
    console.log('Monolith is already launched. Necessary data has been sent. Use the --second-instance flag to bypass the single instance limitation.');
    app.exit()
  }
