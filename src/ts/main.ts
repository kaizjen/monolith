...
app.userAgentFallback = app.userAgentFallback
  .replace(/monolith\/.*? /, '')    // remove the 'monolith/x.y.z'
...
if (userData.lastlaunch.get().launchFailed) {
 ...
 disabled, and the flags (mth://flags) were ignored. Restart Monolith to exit safe mode.")
  ...
  if (app.isPackaged && !userData.lastlaunch.get().exitedSafely) {
    dialog.showErrorBox("Oh no!", "Monolith's work ended abruptly last time. There might be some unsaved data.")
  }
  ...
  if (!userData.control.options.do_not_quit.value) {
    ...
    let img = nativeImage.createFromPath(
      pathModule.join(__dirname, 
        process.platform == 'win32' ? '../../monolith.ico' : '../../monolith.png'
      )
    );
