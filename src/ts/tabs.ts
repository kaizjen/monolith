...
function handleBeforeUnload<T>(wc: WebContents, proceed: () => T): Promise<false | T> {
  ...
  return new Promise(resolve => {
    // This is a race condition. Monolith gives the tab 3 seconds to react to the beforeunload event.
...
export function attach(win: TabWindow, tab: Tab) {
  ...
  tab.webContents.on('page-favicon-updated', async(_e, favicons) => {
    ...
    if (tab.private) {
      ...
      tab.faviconURL = null;
      sendUpdate('favicon', `m-res://${nativeTheme.shouldUseDarkColors ? 'dark' : 'light'}/tab_privatemode.svg`);
      return;
    }
